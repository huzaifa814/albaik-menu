"""Generate the QR codes for AL BAIK.

    python tools/make_qr.py --base https://huzaifa814.github.io/albaik-menu/ --tables 20

Writes into qr/:
    table-01.png ... table-NN.png   menu QR carrying the table number (?t=N)
    menu.png                        plain menu QR, no table
    review.png                      Google review QR
"""
import argparse
import os
import qrcode
from qrcode.constants import ERROR_CORRECT_H

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT = os.path.join(ROOT, "qr")


def write(url: str, path: str) -> None:
    qr = qrcode.QRCode(version=None, error_correction=ERROR_CORRECT_H, box_size=14, border=2)
    qr.add_data(url)
    qr.make(fit=True)
    img = qr.make_image(fill_color="black", back_color="white")
    img.save(path)
    print(f"{os.path.basename(path):<16} {url}")


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--base", required=True, help="site root, e.g. https://user.github.io/albaik-menu/")
    ap.add_argument("--tables", type=int, default=20)
    args = ap.parse_args()

    base = args.base if args.base.endswith("/") else args.base + "/"
    os.makedirs(OUT, exist_ok=True)

    write(base, os.path.join(OUT, "menu.png"))
    write(base + "review.html", os.path.join(OUT, "review.png"))
    for n in range(1, args.tables + 1):
        write(f"{base}?t={n}", os.path.join(OUT, f"table-{n:02d}.png"))


if __name__ == "__main__":
    main()
